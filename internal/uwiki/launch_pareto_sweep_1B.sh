...
# KEEP_CHECKPOINTS, NO_TRAINER_STATE and LR_SCHEDULE are single tokens, so they
# are safe. LR_SCHEDULE is the P2 ablation knob (constant | linear | warmup |
...
for var in TOTAL_BATCH MICRO_BATCH EPOCHS MAX_STEPS HARD_STEP_CAP DTYPE FROZEN_DTYPE GRAD_CKPT MODEL REVISION OLMO_CONFIG START_STEP FORGET_EXPS SEED MAX_SEQ_LEN LR LR_SCHEDULE RMU_LAYER RMU_ALPHA RMU_STEPS RMU_NLAYERS RETAIN_WEIGHT SATIMP_BETA2 SIMNPO_GAMMA OUTPUT_ROOT KEEP_CHECKPOINTS NO_TRAINER_STATE RESUME_OPTIM OPTIM_REPO OPTIM_REVISION; do
  case "${!var:-}" in *,*)
