...
# Evaluate ONE point of the Pareto plot: either one trained sweep cell, or one
# reference anchor (the un-unlearned baseline / the deep-ignorance target).
#
...
#               (default: $HOME/pretrain-experiments/noise-vectors/OLMo-2-1B-Exp)
...
# Exit non-zero if anything failed, so sacct/--dependency can see it.
[ -z "$FAILED" ]
